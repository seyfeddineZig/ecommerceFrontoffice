<template>
  <q-page class="bg-grey-1 q-pa-md" padding>
    <div v-if="getCategoryBySelectedLang" class="row q-pa-md">
      <div class="col-12">
            ...
            <div class="row">
              <div class="col-md-12">
                <div class="text-h6 text-weight-bold q-mb-md">
                  {{ getCategoryBySelectedLang.name }}
                </div>
                <div class="text-caption">
                  {{ getCategoryBySelectedLang.description }}
                </div>
              </div>
            </div>
          </q-card-section>
...
export default {
  ...
  methods: {
    getProducts () {
      this.$axios.get('/productsByCategory' + (this.getAuth === null ? 'ForVisitor' : '') + '/' + this.$route.params.id).then(res => {
